...
export default class Session {
	...
	async getAllInterlocutors() {
		...
		const comments = await golos.api.getAllContentReplies(this._username, await this.getDRE(this._username));
		for (const comment of comments) {
			if (/START_DIALOG.+PUBLIC_KEY: (\d+)/.test(comments.body)
				&& comment.author !== this._username) {
				result.add(comments.author);
			}
		}
