...
export default class Session {
	...
	async _getPubKey(username) {
		...
		for (const comment of comments) {
			...
			console.log("They pubkey: " + anotherPublicKey);
			if (!anotherPublicKey) continue;
			return (this._pubKeys[username] = anotherPublicKey);
		}
		throw 'no connection';
	}
	...
	async isConnectionExists(to) {
		...
		const comments = await golos.api.getAllContentReplies(
			to, await this.getDRE(to),
		);
		for (const comment of comments) {
			if (comment.author !== this._username) continue;
			const anotherPublicKey = (/START_DIALOG[\s.]+PUBLIC_KEY: (\d+)/.exec(comment.body) || [null, null])[1];
			console.log("My pubkey: " + anotherPublicKey);
			if (!anotherPublicKey) continue;
	...
	async getDialog(toUser) {
		...
		if (await this.getReceiver(toUser)) {
			await this.createChannel(toUser);
		}
