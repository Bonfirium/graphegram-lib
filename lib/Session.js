...
import { encryptKey, base62, encryptMessage } from './crypto/Encryption';
...
export default class Session {
	...
	async getDialog(toUser) {
		...
		for (const comment of comments) {
			if (comment.author === this._username
				...
				return (this._dialogs[toUser] = comment.permlink);
			}
		}
	...
	async sendMessage(toUser, message) {
		...
		await this._comment(
			this._username,
			await this.getDialog(toUser),
			dialog + Session._getNewPermlink(),
			'MESSAGE', base62.encode(encryptMessage(message, await this._getKey(toUser))),
		);
